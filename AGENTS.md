...
The build has a `prebuild` hook (`seo:generate`) that regenerates `public/sitemap.xml`.
...
| `config/assets.ts` | optimized image paths and alt text |
| `public/Profile.png` | source profile photo used by `npm run images:optimize` |
